...
const RatingDetail: React.FC = () => {
    ...
    async function getStarData() {
      try {
        ...
        const validStars = data
          ...
          .map((star) => ({
            ...star,
            typeName: StarType[String(star.type)]?.name || String(star.type)
          }));
  ...
  useEffect(() => {
    ...
    if (filterMaster.trim() !== "") {
      filtered = filtered.filter(star => star.masterId.includes(filterMaster.trim()));
    }
  ...
  }, [allStarData, filterMaster, filterType, startDate, endDate]);
  ...
  const columns = [
    { id: "masterId", label: "行編", sortable: true },
