...
import { fetchMaster, fetchStars } from "../api/api";
...
const Dashboard: FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      const [masters, stars] = await Promise.all([fetchMaster(), fetchStars()]);

      // 過濾掉無效的星星記錄
      const validStars = stars.filter((star: any) => star.valid !== false);

      // 僅計算在有效主管名單中的星星記錄
      const validMasterIds = new Set(masters.map((s: any) => s.id));

      let totalStars = 0;
      const masterStars: { [key: string]: number } = {};
      validStars.forEach((star: any) => {
        if (validMasterIds.has(star.masterId)) {
          masterStars[star.masterId] = (masterStars[star.masterId] || 0) + star.score;
      ...
      const avgStars = totalMasters ? (totalStars / totalMasters).toFixed(2) : "0";

      // 針對每位主管計算星星總和的中位數
      const allStarsArray = masters.map((s: any) => masterStars[s.id] || 0);
      ...
      const masterList = masters.map((s: any) => ({
        id: s.id,
        name: s.name,
        region: branchArea[branchList[s.branchCode]?.area] || "N/A",
        department: s.branchCode || "N/A",
        stars: masterStars[s.id] || 0,
      }));

      // 依星星數排序（由高到低）
      masterList.sort((a: { stars: number }, b: { stars: number }) => b.stars - a.stars);

      // 將排名加進資料（依排序順序設定 rank = index+1）
      const rankedMasters = masterList.map((s: any, index: number) => ({
        rank: index + 1,
        ...s,
      }));
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="p-6">
        ...
        <div className="grid grid-cols-5 gap-4 mb-6">
          <StatCard
            title="總達人數"
            value={dashboardData.totalMasters}
        ...
        <div className="mb-6">
          <DataTable
            ...
            data={dashboardData.mastersRank}
            pagination={{
              pageSize: 10,
